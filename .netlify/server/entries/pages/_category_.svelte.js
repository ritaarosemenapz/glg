...
const U5Bcategoryu5D = (0, import_index_27acdd93.c)(($$result, $$props, $$bindings, slots) => {
  ...
  $$result.css.add(css);
  return `${$$result.head += `${$$result.title = `<title>${(0, import_index_27acdd93.e)(CATEGORY.toUpperCase())} | GLG</title>`, ""}`, ""}

<section class="${"container svelte-6l13mn"}"><h2>Latest articles on ${(0, import_index_27acdd93.e)(CATEGORY)}</h2>
	${FILTERED_POSTS.length ? (0, import_index_27acdd93.d)(FILTERED_POSTS, (post) => {
    return `${post ? `<div class="${"card svelte-6l13mn"}"><img class="${"post-cover svelte-6l13mn"}"${(0, import_index_27acdd93.b)("src", post.meta.cover, 0)} alt="${""}">
				<div class="${"card-info svelte-6l13mn"}"><div><h3>${(0, import_index_27acdd93.e)(post.meta.title)}</h3></div>
					${(0, import_index_27acdd93.v)(import_ReadMoreTag_2fe9df27.R, "ReadMoreTag").$$render($$result, { path: post.path.replace(".md", "") }, {}, {})}</div>
			</div>` : ``}`;
